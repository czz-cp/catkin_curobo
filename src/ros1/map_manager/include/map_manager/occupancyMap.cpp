...
namespace mapManager{
	...
	void occMap::initParam(){
		...
		if (not this->nh_.getParam(this->ns_ + "/robot_size", robotSizeVec)){
			robotSizeVec = std::vector<double>{0.15, 0.15, 0.2};  // 机械臂默认尺寸
		}
		else{
	...
	void occMap::staticClusteringCB(const ros::TimerEvent& ){
		...
		// 机械臂环境参数适配
		double groudHeight = -0.1;    // 机械臂工作台高度
		double verticalRange = 1.5;   // 机械臂垂直工作范围
		double horizontalRange = 1.0; // 机械臂水平工作范围
		double cloudRes = 0.1;        // 机械臂环境需要更高精度
	...
	void occMap::getPointcloud(){
		...
		for (int i=0; i<this->projPointsNum_; ++i){
			...
			currPointMap = this->orientation_ * currPointCam + this->position_; // transform to map coordinate
			// 机械臂环境：保留近距离障碍物信息，降低最小距离阈值
			if ((currPointMap-this->position_).norm()>=0.1){
